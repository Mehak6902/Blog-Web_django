...
from .models import blog_category, contact_info, SubscribedUser, blog_post
from .forms import Blog_Form, BlogPost_Form, CommentForm,  blog_post
...
def blog_details(request, blog_id):
    ...
    print("Viewing blog:", obj.id)
    print("Session keys:", request.session.keys())
    
    # Check if the session variable indicating the view has already been set
    if 'viewed_' + str(blog_id) not in request.session:
        # Increment the view count
        obj.view_count += 1
        obj.save()
        # Set the session variable to indicate that the user has viewed this blog
        request.session['viewed_' + str(blog_id)] = True
        print("View count incremented for blog:", obj.id)
